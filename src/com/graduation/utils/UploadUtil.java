...
public class UploadUtil {
    ...
    * */
    public static String uploadOneFile(MultipartFile file, String uploadPath, HttpServletRequest request,String type){
        ...
        if(file.isEmpty()){
            ...
            try {
                ...
                String realUrl = request.getContextPath();//获取到 http://localhost:8080/project1
                System.out.println("realUrl前缀:"+realUrl);
                String serverName = request.getServerName();
                int port = request.getServerPort();

                //获取项目访问上传文件的实际地址  http://localhost:8080/project1/item/image/文件名
    ...
    public static String uploadMutiFile(MultipartFile[] files, String uploadPath, HttpServletRequest request,String type){
    ...
    public static String uploadMutiFileUpdate(MultipartFile[] files, String uploadPath, HttpServletRequest request,
                                              String type,String[] originalUrl){

        //将originalUrl处理   originalUrl地址根据顺序 只会顺序存储
        int size = originalUrl.length;
        System.out.println(size);//注意这里分割后的数据可能会包含[ ]需要去除
        ...
        String itemImageOtherUrl = "";//用于保存其他图片地址
        for(int i=0;i<files.length;i++){  //由于我们设计的其他图片上传，写了5个上传框 这里 length一定是5
            if(files[i].isEmpty()){
                System.out.println("当前位置:"+i+"无文件！");
                //由于originalUrl 按序存储,其他位置如果没有图  不需要处理
                ...
                if(uploadFlag){
                    ...
                    try {
                        files[i].transferTo(dest);   //暂时先不上传
                        //上传成功需要把该文件在服务器中实际访问地址返回
                        String realUrl = request.getContextPath();//获取到 http://localhost:8080/project1
                        System.out.println("realUrl前缀:"+realUrl);
                        String serverName = request.getServerName();
    ...
    //更新商品时，上传单个文件功能
    public static String uploadOneFileUpdate(MultipartFile file, String uploadPath, HttpServletRequest request,
                                             String type,String originalUrl){
        String flag = "";
        String newUrl = "";
        if(file.isEmpty()){
